...
(function () {
    ...
    if (jQuery.browser.msie) {
        ...
        if (jQuery.browser.version < 9)
            _head.append('<script type="text/javascript" src="' + baseURL + 'Scripts/html5shiv.js" />');
    ...
    // Insert stylesheets
    headQueue.pushE_5grid(baseURL + 'Content/core.css');
    headQueue.pushE_5grid(baseURL + 'Content/style.css');
    ...
    (function () {
        ...
        if ((doMobile && (ww <= _settings.breakpoint_mobile || sw <= _settings.breakpoint_mobile)) || doMobileOnly) {
            ...
            headQueue.pushE_5grid(baseURL + 'Content/core-mobile.css');
            _settings.mobileUI = 1;
            if (_settings.mobileUI == 1) {
                _settings.mobileUI_force = 1;
                if (_settings.mobileUI_theme != 'none') {
                    headQueue.pushE_5grid(baseURL + 'Content/mobileUI-' + _settings.mobileUI_theme + '.css');
            ...
            headQueue.pushE_5grid(baseURL + 'Content/style' + '-mobile.css');
        }
        else {
            // Fluid (exclusive)
            if (doFluid) {
                ...
                headQueue.pushE_5grid(baseURL + 'Content/core-desktop.css');
                headQueue.pushE_5grid(baseURL + 'Content/core-fluid.css');
                headQueue.pushE_5grid(baseURL + 'Content/style' + '-fluid.css');
            }
                // Desktop
            else if (doDesktop) {
                _5grid.isDesktop = true;
                headQueue.pushE_5grid(baseURL + 'Content/core-desktop.css');
                headQueue.pushE_5grid(baseURL + 'Content/style' + '-desktop.css');

                // 1200px
                if (ww >= _settings.breakpoint_1000px) {
                    _5grid.is1200px = true;
                    _head.prepend('<meta name="viewport" content="width=' + _settings.viewport_is1200px + '" />');
                    headQueue.pushE_5grid(baseURL + 'Content/core-1200px.css');
                  
                    // Load 1200px stylesheet if 1200px was explicitly enabled
                    if (do1200px)
                        headQueue.pushE_5grid(baseURL + 'Content/style' + '-1200px.css');
                }
                    // 1000px
                else {
                    _5grid.is1000px = true;
                    _head.prepend('<meta name="viewport" content="width=' + _settings.viewport_is1000px + '" />');
                    headQueue.pushE_5grid(baseURL + 'Content/core-1000px.css');

                    // Load 1000px stylesheet if 1000px was explicitly enabled
                    if (do1000px)
                        headQueue.pushE_5grid(baseURL + 'Content/style' + '-1000px.css');
                }
            }
            else {
                // 1000px (exclusive)
